...
export default async function TripsPage() {
    ...
    const sortedTrips = [...trips].sort((a,b)=> new Date(b.startDate).getTime()- new Date(a.endDate).getTime())
        ...
        <Card>
            ...
            <CardContent>
                <p>{trips.length === 0 ? "Start planning your first trip by clicking the button above.":
                    `You have ${trips.length} ${trips.length===1?"trip":"trips"} planned. ${upcomingTrips.length>0 ? upcomingTrips.length + " upcoming.":""}`}</p>
            </CardContent>
        </Card>
            <div>
