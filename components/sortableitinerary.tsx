...
function SortableItem({item}:{item:Location}) {
    ...
    return <div 
    ref={setNodeRef} {...attributes} {...listeners} style={{transform:CSS.Transform.toString(transform),transition}}
    className="p-4 border rounded-md flex justify-between items-center hover:shadow transition:shadow">
        <div>
            ...
            <p className="text-sm text-gray-500 truncate max-w-xs">{`Latitude: ${item.lat}, Longtitude: ${item.lng}`}</p>
        </div>
